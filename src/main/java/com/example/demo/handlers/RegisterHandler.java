...
public class RegisterHandler {
  ...
  public void addPersonalInfo(RegisterModel registerModel,
      PersonalInfo personalInfo) {
    registerModel.setPersonalInfo(personalInfo);
  }

  public void addBillingInfo(RegisterModel registerModel,
...
//		transitionValue = "failure";
  ...
  public String validatePersonalInfo(PersonalInfo personalInfo,
      MessageContext error) {
    String transitionValue = "success";

    // Checking that username is not equal to 'Vakho' :d XXX do whatever you
    // want!
    if (personalInfo.getUsername().equalsIgnoreCase("frozar")) {
      error.addMessage(new MessageBuilder(). //
          error() //
          .source("username") //
          .defaultText("You are not allowed to use frozar as the username!") //
          .build());
